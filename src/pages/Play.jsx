...
function Play() {
    ...
    return (
        ...
        <div className={visible ? "background" : "background2"}>
           ...
           <div style={{height:"20%",width:"30%",marginTop:"20%",marginLeft:"50%",display:"flex",position:"absolute"}}>
                <div className="row"  >
                    <div className="row-6 " >
                    {visible &&
                     <input placeholder="Enter Subject" className='input_name' value={sub} onChange= {(event)=>setSub(event.target.value)}></input>}
                    {/* //  <select placeholder="Enter Subject" className='input_name' >
                    //     {data.map((item)=>{ return <options value={item[0]}>{item[0]}</options>})}
                    // </select>} */
                    }
                    </div>
                    <div className="row-2 " >
                    {visible && <input placeholder="Enter Your Name" className="input_name" value={name} onChange= {(event)=>setName(event.target.value)}></input>}
